...
import 'gTranslate.dart';
import 'global.dart';

class TextReading extends StatefulWidget {
  var indexOfTitles;

  TextReading(this.indexOfTitles);

  @override
  State<StatefulWidget> createState() {
    return TextReadingState(indexOfTitles);
  }
}
...
class TextReadingState extends State<TextReading> {
  ...
  TextReadingState(this.indexOfTitles);
  ...
  Color buttonsColor = Colors.deepOrangeAccent;
  static ValueNotifier adNotifier = ValueNotifier(0);
  AdsUnits adsUnits = AdsUnits();

  scrollAtStart() {
    if (scrollController.isAttached) {
      scrollController.scrollTo(index: indexOfTitles, duration: Duration(milliseconds: 100));
    } else {
      Future.delayed(Duration(milliseconds: 10), scrollAtStart);
    }
  }
  ...
  Widget build(BuildContext ctx) {
    ...
    this.ctx = ctx;
    title = Global.languageTitleMap[Global.settings['language']];
    var titleEnglish = Global.languageTitleMap['english'];
  ...
  speakButtonClick(i) async {
    try {
      ...
      // if (Globle.settings['language'] == Globle.language[2]) {
      //   await flutterTts.setSpeechRate(0.6);
      //   await flutterTts.setVoice({'name': 'hi-in-x-cfn-local', 'locale': 'hi-IN'});
      // } else if (Globle.settings['language'] == Globle.language[1]) {
      ...
      flutterTts.setCompletionHandler(() {
        if (i < dataBox.length - 1) {
          ...
          speakButtonClick(versePlayingIndex);
          listNotifier.value++;
  ...
  loadData() async {
    dataBox = await Hive.openBox('gita${Global.settings['language']}');
    translateBox = await Hive.openBox('gita${Global.settings['translateLanguage']}');

    for (int i = 0; i < dataBox.length; i++) {
      if (dataBox.getAt(i)['type'] == 'title') {
  ...
  gTranslate(i) async {
    try {
      ...
      if (result.isNotEmpty && result[0].rawAddress.isNotEmpty) {
        ...
        var box = await Hive.openBox(('gita' + Global.language[1]));
                            ...
                            Expanded(
                                ...
                                child: Container(
                                  ...
                                  child: Row(
                                    children: [
                                      Text(
                                        Global.languageTitleMap[Global.settings['language']]
                                                .toString()
                                                .capitalizeFirstofEach +
                                            ' Bookmarks',
                                        style: TextStyle(fontWeight: FontWeight.w600, color: Colors.black87),
          ...
          return Dialog(
            child: ListView(
              ...
              children: [
                ...
                //   child: Text(
                //     title,
                //     style: TextStyle(fontSize: 15, fontWeight: FontWeight.w500, color: Colors.deepOrangeAccent),
                //   ),
                // ),
                      ...
                      child: Flex(
                        ...
                        children: [
                          Expanded(
                            ...
                            child: ListTile(
                              title: Text(
                                dataBox.getAt(mapFitered.keys.toList()[i])['data'].toString().split('-')[0].trim(),
                                style: TextStyle(fontSize: 20, fontWeight: FontWeight.w600),
                              ),
                              subtitle: Text(
                                dataBox.getAt(mapFitered.keys.toList()[i])['data'].toString().split('-')[1].trim(),
                                //style: TextStyle(fontSize: 20, fontWeight: FontWeight.w600),
                              ),
                            ),
                          ),
